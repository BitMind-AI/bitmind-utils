...
def create_random_mask(
    ...
    allowed_shapes: List[str] = ["rectangle", "circle", "ellipse", "triangle"],
    feather_range: Tuple[int, int] = (0, 0),  # ignored
    edge_bias: float = 0.2,
) -> Tuple[Image.Image, List[dict]]:
    """
    Create a random mask (or masks) for i2i/inpainting with more variety.
    Returns the mask and a list of metadata dicts for each mask.
    """
    w, h = size
    ...
    for attempt in range(max_retries):
        ...
        for _ in range(n_masks):
            ...
            if shape == "circle":
                ...
                meta = {"shape": shape, "center": (cx, cy), "radius": r, "bbox": (cx - r, cy - r, cx + r, cy + r)}
                draw.ellipse([cx - r, cy - r, cx + r, cy + r], fill=255)
            elif shape == "rectangle":
                x = np.random.randint(0, w - width + 1)
                y = np.random.randint(0, h - height + 1)
                meta = {"shape": shape, "bbox": (x, y, x + width, y + height)}
                draw.rectangle([x, y, x + width, y + height], fill=255)
            elif shape == "ellipse":
                x = np.random.randint(0, w - width + 1)
                y = np.random.randint(0, h - height + 1)
                x0, y0, x1, y1 = x, y, x + width, y + height
                meta = {"shape": shape, "bbox": (x0, y0, x1, y1)}
                ...
                x = np.random.randint(0, w - width + 1)
                y = np.random.randint(0, h - height + 1)
                # Place points at the three corners of the bounding box, with small jitter
                ...
                meta = {"shape": shape, "bbox": (x, y, x + width, y + height), "points": pts}
            mask_metadata.append(meta)
        if np.array(mask).max() > 0:
            mask_rgb = mask.convert("RGB")
            return mask_rgb, mask_metadata
    mask_rgb = mask.convert("RGB")
    return mask_rgb, mask_metadata
