...
def main():
    parser = argparse.ArgumentParser(description="Download all files from an S3 directory, preserving structure.")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.workers) as executor:
        list(tqdm(executor.map(download_wrapper, files_to_download), total=len(files_to_download), desc="Downloading files"))
